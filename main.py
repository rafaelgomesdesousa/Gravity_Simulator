...
while running:
    ...
    if state==0:
        btn_simular=criar_botoes(screen, 250, 75, meio_x-125, meio_y-275, 'black', 'Simular')
        btn_simulacoes=criar_botoes(screen, 250, 75, meio_x-125, meio_y-75, 'black', 'Simulacoes Prontas')
            ...
            if (event.type==pygame.MOUSEBUTTONDOWN):
                if event.button==1:
                    if btn_simular.collidepoint(event.pos):
                        state=1
                    if btn_simulacoes.collidepoint(event.pos):
                        state=2
